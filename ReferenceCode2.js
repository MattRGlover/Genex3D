...
const SHAPE_SPEED_MIN = 0.00167; // Corresponds to a ~10-second animation (1/600)
...
function draw() {
  ...
  if (compositionFinished && resetCycleTimestamp === 0 && millis() - completionTimestamp > 20000) {
  ...
  if (resetCycleTimestamp > 0) {
    ...
    if (elapsed < FADE_DURATION_MS) {
      ...
      // 3. Fading in
      if (!resetHasOccurred) {
        console.log(`[DEBUG] Blackout complete. Fading in at ${millis()}`);
        reset();
...
// —————————————————————————————————————
function handleDrag() {
  if (totalElementsCreated >= MAX_ELEMENTS) return;
  let now = millis();
...
class SpiralAnim {
  constructor(x, y, opts) {
    this.x = x;
    this.y = y;
    this.steps = opts.steps || 200; // Increased steps for smoothness
    ...
    this.w = opts.strokeWeight || random(BASE_UNIT * 0.002, BASE_UNIT * 0.005); // Thicker stroke
    ...
    for (let i = 0; i <= this.steps; i++) {
      ...
      this.sv.push(createVector(sx, sy)); // Store as p5.Vector
    }
  }
  ...
  step(g) {
    ...
    // To create a tapered line, we draw individual segments with changing stroke weights.
    const numPointsToShow = floor(this.progress);

    for (let j = 1; j < numPointsToShow && j < this.sv.length; j++) {
      // The weight is tapered along the *entire* length of the spiral.
...
class LatticeAnim {
  ...
  step(g){
    if (this.cIdx >= this.cells.length) return false; // Animation is done
...
class KandinskyShape {
  constructor(x,y,opts={}){
    ...
    this.index      = ++shapeCounter;
        let base        = opts.size || random(BASE_UNIT * 0.08, BASE_UNIT * 0.4);
    let sf;
    if (this.index <= 2) {
...
function reset() {
  ...
  maxElementsReached = false; // Reset the new flag
  shapeCounter = 0;
