...
// Firebase variables
let db, storage;
let firebaseUploadComplete = false;
...
function checkCompletion() {
  ...
  if (currentElementCount >= 50) {
    if (!compositionFinished) {
      ...
      // Start the fade sequence after 10 seconds of viewing
      setTimeout(() => {
        console.log('🎭 Starting fade sequence and Firebase upload...');
        fadeStartTime = millis();
        
        // Capture and upload pixel bitmap to Firebase
        savePixelBitmapToFirebase();
        
        // Start reset cycle after upload completes
        setTimeout(() => {
          if (firebaseUploadComplete) {
            console.log('🔄 Starting reset cycle...');
            resetCycleTimestamp = millis();
            
            // Reset after 5 seconds of blackout
            setTimeout(() => {
              reset();
              randomSeedValue = int(random(1000000));
              randomSeed(randomSeedValue);
              generateWatercolorBackground(finalBgLayer);
              console.log('✨ Fresh composition cycle started');
            }, 5000);
          }
        }, 2000); // Wait 2 seconds for upload to complete
        
      }, 10000); // 10 second viewing period
    }
  }
}
...
function handleTouchStart(event) {
    ...
    pmouseX = mouseX;
    pmouseY = mouseY;
    prevTouch = { x: mouseX, y: mouseY, moved: false };
    handleDrag();
}
  ...
  constructor(x,y,opts={}){
    ...
    if (this.index <= 2) {
      ...
      let styles = opts.styleSet || [
        ...
        "openRect", "openRect", "openRect", "openRect",
        "openTriangle", "openTriangle", "openTriangle", "openTriangle",
        "openSemiCircle", "openSemiCircle", "openSemiCircle", "openSemiCircle",
...
function reset() {
  ...
  // Reset Firebase upload state
  firebaseUploadComplete = false;
  capturedPixelBitmap = null;
  fadeStartTime = 0;
...
// —————————————————————————————————————
// PIXEL BITMAP CAPTURE & FIREBASE UPLOAD
// —————————————————————————————————————
...
function capturePixelBitmap() {
  ...
  console.log('🔍 Pixel capture debug:', {
  ...
  console.log(`✅ Pixel bitmap captured: ${actualPixelWidth}x${actualPixelHeight} = ${pixelData.length} pixels (density: ${density}x)`);
  
  return bitmap;
}

async function savePixelBitmapToFirebase() {
  if (!db || !storage) {
  ...
  try {
    console.log('🎯 Starting pixel bitmap save to Firebase...');
    ...
    capturedPixelBitmap = capturePixelBitmap();
    
    // Convert to JSON blob for Storage upload
    const pixelBitmapJson = JSON.stringify(capturedPixelBitmap);
    const pixelBitmapBlob = new Blob([pixelBitmapJson], { type: 'application/json' });
    const sizeMB = pixelBitmapBlob.size / (1024 * 1024);
    
    console.log(`📏 Pixel bitmap blob size: ${sizeMB.toFixed(2)}MB`);
    
    // Upload pixel bitmap to Firebase Storage
    console.log(`📤 Uploading pixel bitmap to Firebase Storage...`);
    const storageRef = storage.ref();
    const pixelBitmapRef = storageRef.child(`pixelBitmaps/${compositionId}.json`);
    
    const uploadTask = await pixelBitmapRef.put(pixelBitmapBlob);
    const pixelBitmapUrl = await uploadTask.ref.getDownloadURL();
    
    console.log(`✅ Pixel bitmap uploaded to Storage: ${pixelBitmapUrl}`);
    
    // Create scene report with pixel bitmap data for gallery rendering
    const sceneReport = {
      id: compositionId,
      captureMethod: 'pixel_bitmap',
      version: '2.0',
      pixelBitmapUrl: pixelBitmapUrl, // Reference to Storage
      // pixelBitmap: capturedPixelBitmap, // Removed to stay under Firestore 1MB limit
      pixelBitmapSize: {
        width: capturedPixelBitmap.width,
        height: capturedPixelBitmap.height,
        pixelCount: capturedPixelBitmap.pixels.length,
        fileSizeMB: sizeMB
      },
      canvasSize: {
        width: width,
      ...
      canvasWidth: width, // Add for gallery compatibility
      canvasHeight: height, // Add for gallery compatibility
      // Device/platform metadata
      deviceInfo: {
        platform: 'mobile',
        source: 'sketchIOS2.0.js',
        userAgent: navigator.userAgent,
        screenSize: {
          width: screen.width,
          height: screen.height
        },
        canvasOrientation: width > height ? 'landscape' : 'portrait',
        pixelDensity: pixelDensity()
      },
      createdAt: firebase.firestore.FieldValue.serverTimestamp()
    };
    
    console.log(`🔍 Scene report metadata prepared:`, {
      id: sceneReport.id,
      captureMethod: sceneReport.captureMethod,
      pixelBitmapUrl: sceneReport.pixelBitmapUrl,
      bitmapWidth: sceneReport.pixelBitmapSize.width,
      bitmapHeight: sceneReport.pixelBitmapSize.height,
      pixelCount: sceneReport.pixelBitmapSize.pixelCount,
      version: sceneReport.version
    });
    
    // Save scene report metadata to Firestore
    console.log(`📋 Saving metadata to Firestore...`);
    await db.collection('sceneReports').doc(compositionId).set(sceneReport);
    
    console.log(`✅ Pixel bitmap composition saved successfully!`);
    console.log(`📊 Storage: ${pixelBitmapUrl}`);
    console.log(`📋 Firestore: sceneReports/${compositionId}`);
    
    // Signal that Firebase upload is complete
    firebaseUploadComplete = true;
    
  } catch (error) {
    console.error('❌ Error saving pixel bitmap composition to Firebase:', error);
    firebaseUploadComplete = true; // Set flag so reset can proceed even with errors
  }
}