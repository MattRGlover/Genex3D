...
let firebaseUploadComplete = false; // Track when Firebase upload is done
...
function draw() {
  ...
  if (compositionFinished && resetCycleTimestamp === 0 && millis() - completionTimestamp > 10000) {
    ...
    // Start reset cycle but don't reset until Firebase upload completes
  ...
  if (resetCycleTimestamp > 0) {
    ...
    if (elapsed < FADE_DURATION_MS) {
      ...
      // 2. Blackout period - reset happens here while screen is black, but only after Firebase upload completes
      fadeAlpha = 255;
      if (!resetHasOccurred && firebaseUploadComplete) {
        console.log(`[DEBUG] Resetting during blackout at ${millis()} - Firebase upload complete`);
        reset();
        resetHasOccurred = true;
        firebaseUploadComplete = false; // Reset flag for next cycle
      }
...
function captureShapeReport(shape) {
  const report = {
    ...
    gradientAngle: shape.gradientAngle || null // Ensure gradientAngle is always defined
  };
...
function reset() {
  ...
  // Capture background as base64 image data for exact reproduction
  sceneReport.backgroundImageData = finalBgLayer.canvas.toDataURL();
  
  console.log("System reset. All state variables and sceneReport have been initialized.");
  console.log("🎨 Background captured for pixel-perfect reproduction");
}
...
function initializeFirebase() {
  if (typeof firebaseConfig !== 'undefined') {
    ...
    console.warn('Firebase config not found. Scene reports will not be saved.');
    firebaseInitialized = false;
  }
}

async function saveCompositionToFirebase() {
  if (!firebaseInitialized) {
    console.warn('Firebase not initialized. Cannot save composition.');
    return;
  }
  ...
  try {
    ...
    const highResImage = await generateHighResImage();
    
    // Upload image to Firebase Storage
    const imageUrl = await uploadImageToStorage(highResImage, compositionId);
    
    // Debug: Log scene report before uploading
    console.log(`🔍 PRE-UPLOAD DEBUG - Scene Report Contents:`);
    console.log(`  - Skeletons: ${sceneReport.skeletons.length} items`);
    console.log(`  - Ornaments: ${sceneReport.ornaments.length} items`);
    console.log(`  - Lines: ${sceneReport.lines.length} items`);
    console.log(`  - Beziers: ${sceneReport.beziers.length} items`);
    console.log(`  - Lattices: ${sceneReport.lattices.length} items`);
    console.log(`  - Full sceneReport object:`, sceneReport);
    
    // Check for undefined values in the scene report
    const undefinedFields = findUndefinedFields(sceneReport, 'sceneReport');
    if (undefinedFields.length > 0) {
      console.error('❌ UNDEFINED FIELDS DETECTED:', undefinedFields);
      console.error('❌ Cannot upload to Firebase with undefined values');
      return; // Exit early to prevent Firebase error
    } else {
      console.log('✅ No undefined fields detected in scene report');
    }
    
    // Prepare scene report with image URL and metadata
    const finalReport = {
      ...sceneReport,
      id: compositionId,
    ...
    console.log(`🔍 FINAL REPORT DEBUG:`, finalReport);
    
    // Save scene report to Firestore
    await db.collection('sceneReports').doc(compositionId).set(finalReport);
    
    console.log(`Composition saved successfully with ID: ${compositionId}`);
    console.log(`Image URL: ${imageUrl}`);
    console.log('Scene report data:', finalReport); // Debug log to verify shape data
    
    // Signal that Firebase upload is complete
    firebaseUploadComplete = true;
    
  } catch (error) {
    console.error('Error saving composition to Firebase:', error);
    // Still set flag to true so reset can proceed even if upload fails
    firebaseUploadComplete = true;
  }
}
...
async function generateHighResImage() {
  return new Promise((resolve) => {
    ...
    // Determine high-res dimensions (minimum 4000px on smaller dimension)
    const aspectRatio = width / height;
    let highResWidth, highResHeight;
    
    if (width <= height) {
      highResWidth = 4000;
      highResHeight = Math.round(4000 / aspectRatio);
    } else {
      highResHeight = 4000;
      highResWidth = Math.round(4000 * aspectRatio);
    }
    
    // Create high-res graphics buffer
    const highResGraphics = createGraphics(highResWidth, highResHeight);
    const scaleFactor = highResWidth / width;
    
    // Scale and redraw the composition
    highResGraphics.push();
    highResGraphics.scale(scaleFactor);
    
    console.log(`🖼️ High-res image generation (${highResWidth}x${highResHeight}, scale: ${scaleFactor.toFixed(2)})`);
    
    // Draw background
    console.log(`  🎨 Drawing background layer (${finalBgLayer ? 'exists' : 'missing'})`);
    highResGraphics.image(finalBgLayer, 0, 0);
    
    // Draw line layer
    console.log(`  📝 Drawing line layer (${lineLayer ? 'exists' : 'missing'})`);
    highResGraphics.image(lineLayer, 0, 0);
    
    // Draw shapes directly (like main canvas)
    console.log(`  🔸 Drawing ${skeletons.length} skeleton shapes`);
    skeletons.forEach((s, i) => {
      console.log(`    Skeleton ${i + 1}: type=${s.type}, pos=(${s.x.toFixed(1)}, ${s.y.toFixed(1)}), size=${s.targetSize.toFixed(1)}`);
      s.display(highResGraphics);
    });
    
    console.log(`  🔹 Drawing ${ornaments.length} ornament shapes`);
    ornaments.forEach((o, i) => {
      console.log(`    Ornament ${i + 1}: type=${o.type}, pos=(${o.x.toFixed(1)}, ${o.y.toFixed(1)}), size=${o.targetSize.toFixed(1)}`);
      o.display(highResGraphics);
    });
    
    // Draw foreground layer (lattices, etc.)
    console.log(`  🎆 Drawing foreground layer (${foregroundLayer ? 'exists' : 'missing'})`);
    highResGraphics.image(foregroundLayer, 0, 0);
    
    highResGraphics.pop();
    
    // Convert to blob
    const canvas = highResGraphics.canvas;
    canvas.toBlob((blob) => {
      console.log(`High-res image generated: ${highResWidth}x${highResHeight}`);
      resolve(blob);
    }, 'image/png', 0.95);
  });
}

async function uploadImageToStorage(imageBlob, compositionId) {
  return new Promise((resolve, reject) => {
    console.log('Uploading image to Firebase Storage...');
    
    const storageRef = storage.ref();
    const imageRef = storageRef.child(`compositions/${compositionId}.png`);
